...
export default function HorizontalUpperBar(): JSX.Element {
  return (
    <div className={`${secondary.className} h-10 flex w-full has-[:checked]:bg-gray-300 dark:has-[:checked]:bg-gray-900 relative`}>
      <label className="my-auto ml-auto mr-4" htmlFor="HamburgerMenu">
        ...
        />
        <div
        className="w-full p-2 hidden peer-checked:flex flex-col gap-2 justify-center rounded-b-3xl md:rounded-b-2xl bg-gray-300 dark:bg-gray-900 absolute right-0 top-full md:max-w-48"
        >
          <div className="px-4 py-2 flex bg-gray-500 text-gray-300 dark:text-gray-900 rounded-full">
            <button className="font-bold">English</button>
            <SwitchThemeButton
            classNames={{
              overwriteButton: "ml-auto text-inherit dark:text-inherit",
            }}
            />
          </div>
